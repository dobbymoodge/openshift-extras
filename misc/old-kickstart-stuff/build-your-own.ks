...
synchronize_clock()
{
  ...
  for clock in $(host clock.corp.redhat.com | awk '{ print $NF }')
  do
...
{
  yum install -y \
   rubygem-stickshift-node rubygem-gearchanger-mcollective-plugin \
   rubygem-swingshift-mongo-plugin rubygem-uplift-bind-plugin rhc \
   mcollective mcollective-qpid-plugin stickshift-broker \
   stickshift-mcollective-agent stickshift-port-proxy
}
...
install_node_pkgs()
{
  # We have no node-specific packages.
  :
}
...
fix_selinux_contexts()
{
  ...
  restorecon -r /var/lib/stickshift /etc/stickshift/stickshift-node.conf /etc/httpd/conf.d/stickshift
}
...
{
  # We actually need a superset of the services needed on a node, so
  # avoid some repetition by re-using the enable_services_on_node
  # function.
  enable_services_on_node
...
EOF
  ...
  # Inhibit NetworkManager from modifying the configuration.
...
#
# Parse the kernel command-line, set up an associative array with the
# parameters specified on it, and define functions broker() and node(),
# which return true or false as appropriate based on whether we are
# configuring the host as a broker or as a node.
#

# Define the cmdline associative array, which will provide a simple way
# to get settings from the boot command-line.
declare -A cmdline # Declare a BASH4 associative array.
...
# or as a node.
[[ ${cmdline[install_broker]} =~ (1|true) ]] \
 && broker() { :; } \
 || broker() { false; }

[[ ${cmdline[install_node]} =~ (1|true) ]] \
 && node() { :; } \
 || node() { false; }
...
domain="${cmdline[domain]:-example.com}"

# The hostname of the broker.
broker_hostname="${cmdline[broker_hostname]:-broker}"

# The hostname of the node.
node_hostname="${cmdline[node_hostname]:-node}"
...
broker_ip_addr="${cmdline[broker_ip_addr]:-${broker_ip_addr:-$(/sbin/ip addr show dev eth0 | awk '/inet / { split($2,a,"/"); print a[1]; }')}}"

# The IP address of the node.  If no IP address is given on the kernel
# command-line, the IP address of the current host is used.
node_ip_addr="${cmdline[node_ip_addr]:-${node_ip_addr:-$(/sbin/ip addr show dev eth0 | awk '/inet / { split($2,a,"/"); print a[1]; }')}}"
